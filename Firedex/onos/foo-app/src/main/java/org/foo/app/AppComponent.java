...
public class AppComponent {
    ...
    private void processPing(PacketContext context, Ethernet eth) {
    	if ( eth == null ) {
    		...
    		TrafficSelector selector = DefaultTrafficSelector.builder()
                    .matchIPProtocol(IPv4.PROTOCOL_TCP).matchIPProtocol(IPv4.PROTOCOL_UDP).build();
    ...
    private class PingPacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
        	...
        	if ( ((IPv4) eth.getPayload()).getProtocol() == IPv4.PROTOCOL_TCP ||
        			((IPv4) eth.getPayload()).getProtocol() == IPv4.PROTOCOL_UDP )
        		processPing(context, null);
            
        	else if (isIcmpPing(eth)) {
